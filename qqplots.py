...
N = np.load(s.nodes_fullname_inf.format(c='c', f='s', a=0.80, g=1.00, b=np.inf))
avg_L = np.mean(N.f.load[18])
DK = N.f.balancing[18]
...
ax1.plot(gevs, cum_dens, '.', markersize=8, label='QQ')
orange = '#FFC400'

ax1.plot([0.9, 1.0], [0.9, 1.0], linewidth=2.0)
...
# new_ticks = [x/(avg_L/1000) for x in new_ticks]
...
new_y.set_ylabel(r'$G_n^B / \langle L_n \rangle$', fontsize=18)
...
ax2.plot(q_bins, bins, '.', markersize=8)
#ax2.plot(q_bins[q_bins > 0.99][0], bins[q_bins > 0.99][0], 'g.', markersize=10)
ax2.plot([min(q_bins), max(q_bins)], [min(q_bins), max(q_bins)], linewidth=2.0)
ax2.set_xlabel(r'GEV quantile $\left[\langle L_n \rangle \right]$', fontsize=18)
ax2.set_ylabel(r'Observed quantile $\left[\langle L_n \rangle \right]$', fontsize=18)
ax2.grid(True)
fig.savefig(s.results_folder + 'qq_plot_DE.pdf', bbox_tight=True)
# fig.savefig('test.pdf', bbox_tight=True)
